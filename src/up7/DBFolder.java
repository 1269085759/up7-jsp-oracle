...
public class DBFolder {
	...
	{
		String sql = "update up7_files set f_perSvr=?,f_lenSvr=? where f_idSvr=?;";
		DbHelper db = new DbHelper();
	...
	static public void Remove(int id_file,int id_folder,int uid)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("begin ");
		sb.append("update up7_files set f_deleted=1 where f_idSvr=? and f_uid=?;");
		sb.append("update up7_files set f_deleted=1 where f_pidRoot=? and f_uid=?;");
		sb.append("update up7_folders set fd_delete=1 where fd_id=? and fd_uid=?;");
		sb.append(" end;");		
		DbHelper db = new DbHelper();
	...
	{
		String sql = "select sum(f_lenSvr) as lenPosted from (select distinct f_md5,f_lenSvr from up7_files where f_pidRoot=? and length(f_md5)>1) a";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
		try {
