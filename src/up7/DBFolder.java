...
public class DBFolder {
	...
	static public void Remove(int id_file,int id_folder,int uid)
	{
		StringBuilder sb = new StringBuilder();
		sb.append("begin ");
		sb.append("update up6_files set f_deleted=1 where f_idSvr=? and f_uid=?;");
	...
	{
		String sql = "select sum(f_lenSvr) as lenPosted from (select distinct f_md5,f_lenSvr from up6_files where f_pidRoot=? and length(f_md5)>1) a";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
		try {
