...
public class fd_appender 
{
    ...
    void make_ids() throws SQLException
    {
        ...
        int[] ids = arr_f.getIntArray();
        this.f_ids = new String[ids.length];
        for(int i = 0 ,l=ids.length;i<l;++i) this.f_ids[i] = String.valueOf(ids[i]);
        
        ids = arr_fd.getIntArray();
        this.fd_ids = new String[ids.length];
        for(int i = 0 ,l=ids.length;i<l;++i) this.fd_ids[i] = String.valueOf(ids[i]);
    ...
    void set_ids()
    {
        ...
        this.m_root.idSvr = Integer.parseInt(this.f_ids[this.f_ids.length-1].trim());//取最后一个
        this.m_root.fdID = Integer.parseInt( this.fd_ids[this.fd_ids.length-1].trim()); //取最后一个
