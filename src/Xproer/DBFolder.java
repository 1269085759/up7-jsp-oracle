...
public class DBFolder {
	...
	static public int Add(FolderInf inf)
	{
		...
		sb.append("insert into hup_folders(");
		sb.append("fd_id");
		sb.append(",fd_name");
		sb.append(",fd_pid");
		sb.append(",fd_uid");
	...
	{
		String sql = "update hup_folders set fd_complete=1 where fd_id=? and fd_uid=?";
		DbHelper db = new DbHelper();
	...
	{
		String sql = "update hup_folders set fd_delete=1 where fd_id=? and fd_uid=?";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
		try 
		{
	...
	static public String GetFilesUnComplete(int fid,FolderInf root)
	{
		...
		sb.append(" from hup_folders where fd_id=?");
	...
	{
		String sql = "select sum(f_PostedLength) as lenPosted from (select distinct f_md5,f_PostedLength from hup_files where f_pidRoot=? and length(f_md5)) a";
		DbHelper db = new DbHelper();
		PreparedStatement cmd = db.GetCommand(sql);
		try {
